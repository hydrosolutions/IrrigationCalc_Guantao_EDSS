...
data_db <- dbConnect(drv = RSQLite::SQLite(), dbname="data-ndvi/guantao-ndvi.sql")
...
init_shp<-readOGR("data/shapefiles/NAME_3_Guantao.shp")
bbox = extent(init_shp)

## Define & Load local meteodata
datafiles<-list("data/meteodata/GUANTAO_ST.csv",
                "data/meteodata/user_dummy.csv")
names<-list("guantaostationlabel","userstationlabel")
lats<-list(36.53313,36.6)
lons<-list(115.3111,115.25)
alt <- list(57,57)
...
## Record App access in logfile. TODO: This feature does not work with SSL. Need to figure out why
