...
shinyServer(function(input, output, session) {
  ...
  output$mymap <- renderLeaflet({
    lat <- meteo_df$LAT
    lon <- meteo_df$LON
    pp <- paste("[",1:2,"] ",tr(meteo_df$NAME),sep="")
  ...
  output$editableClimateTable <- renderRHandsontable({
    data2show <- reactiveClimate$climate[[2]]
  ...
  output$meteo_choices2 <- renderUI({
    ...
    radioButtons("meteo_choice", label = "", choices = choices, selected=default)  
  })
 ...
 #------------- IRRIGATION PLANNING TABLE -------------------------------
  ...
  output$jlkbfakbfk <- renderUI({
    choices <- dbGetQuery(data_db,"SELECT ID FROM settings")$ID
    names(choices) <- tr(dbGetQuery(data_db,"SELECT name FROM settings")$name)
    selectInput('region',
                tr("regiontext"),
                choices = choices)
  })
  ...
  observeEvent(input$recalculateET, {
    reactiveClimate$location[[2]]$LAT <- input$stationLatitude
    reactiveClimate$location[[2]]$LON <- input$stationLongitude
    reactiveClimate$location[[2]]$ALT <- input$stationAltitude
    if (!is.null(input$editableClimateTable)) {
      reactiveClimate$climate[[2]] <- hot_to_r(input$editableClimateTable)
      names(reactiveClimate$climate[[2]]) <- c("MONTH","WETD","PRCP","PRCP_25",
                                               "TMAX","TMIN","HUM","SUNH","WND","ET")
      reactiveClimate$climate[[2]]$RAD <- monthly.radiation(reactiveClimate$climate[[2]],
                                                            reactiveClimate$location[[2]])
      reactiveClimate$climate[[2]]$ET <- as.numeric(monthly.PenmanFAO(reactiveClimate$climate[[2]],
                                                                      reactiveClimate$location[[2]]))
    }
  })
  ...
  observeEvent(input$plantingdate,{
   ...
   results_daily <- reactive({
     ...
     out <- monthly.soil_water_balance(cropparam, soilparam, climate, 
                                       plantingdate, irrigation_threshold=1, 
                                       irrigation_method=selected_strategy)
   })
   ...
   observeEvent(input$okadditnow, {
     ...
     summary <-
       data.frame(
         ...
         area = area_temp,
         conveyanceefficiency = round(input$conveyanceefficiency2, digits=2),
         applicationefficiency = round(input$applicationefficiency2, digits=2),
     ...
     irrigationplanning$colors <- sample(grDevices::colors()[grep('gr(a|e)y', grDevices::colors(), invert = T)], 
    ...
    output$NDVI <- renderLeaflet({
      withProgress(message = 'Reading Data ...', {
        ...
        NDVI %>% addTiles() %>% addRasterImage(
          ...
          addPolygons(data = init_shp,
                      fill = FALSE,
                      color = " #2E2D3A",
                      weight=3, opacity=0.8, options = pathOptions(clickable = FALSE))
      })
    })
  })
